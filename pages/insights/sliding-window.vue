...
import { slidingWindowGuide } from '~/data/sliding-window-guide'
import type { GuideChapter, Difficulty } from '~/data/python-guide'

const activeChapterId  = ref(slidingWindowGuide[0].id)
...
const activeChapter = computed<GuideChapter>(
  () => slidingWindowGuide.find(c => c.id === activeChapterId.value) ?? slidingWindowGuide[0]
)
...
const visibleSections = computed(() => {
  const pool = isFiltering.value
    ? slidingWindowGuide.flatMap(c => c.sections)
...
const totalMustKnow = computed(() =>
  slidingWindowGuide.flatMap(c => c.sections).filter(s => s.mustKnow).length
)
...
function expandAll()  { expandedSections.value = new Set(visibleSections.value.map(s => s.id)) }
function collapseAll() { expandedSections.value = new Set() }
...
useSeoMeta({
  title: 'Sliding Window — DSA Interview Guide',
  description: 'Comprehensive Sliding Window reference for coding interviews — fixed/dynamic windows, templates, 12 LeetCode problems with dry runs, mock interview, and cheat sheets.',
})
</script>
  ...
  <div class="space-y-8">
    ...
    <header class="space-y-4">
      ...
      <div class="space-y-2">
        <div class="flex flex-wrap items-center gap-2">
          ...
          <span class="inline-flex items-center rounded-full border border-slate-200/70 bg-white/70 px-3 py-1 text-xs font-medium text-slate-500 dark:border-slate-700/80 dark:bg-slate-900/50 dark:text-slate-400">
            {{ slidingWindowGuide.length }} chapters
          </span>
        </div>
        <h1 class="section-title text-slate-900 dark:text-white">
          Sliding Window — DSA Interview Guide
        </h1>
        <p class="section-lead max-w-3xl dark:text-slate-300">
          A comprehensive case study on the Sliding Window pattern — from brute force to O(n), covering fixed and dynamic windows, 12 classic problems with full dry runs, reusable templates in 4 languages, a mock interview simulation, and a FAANG-ready practice roadmap.
        </p>
      </div>
    </header>
    ...
    <div class="grid gap-6 lg:grid-cols-[240px_1fr] lg:items-start">
      ...
      <nav class="lg:sticky lg:top-24 space-y-1">
        ...
        <button
          v-for="chapter in slidingWindowGuide"
          :key="chapter.id"
