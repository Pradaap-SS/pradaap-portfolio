...
<template>
  <div v-if="project" class="space-y-12">
    <section class="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
      <div class="space-y-4">
        <p class="text-xs uppercase tracking-[0.2em] text-slate-500">Case Study</p>
        <h1 class="font-display text-4xl font-semibold">{{ project.title }}</h1>
        <p class="text-lg text-slate-600 dark:text-slate-300">{{ project.longDescription }}</p>
        <div class="flex flex-wrap gap-3">
          <span class="chip">{{ project.role }}</span>
          <span class="chip">{{ project.period }}</span>
        </div>
        <div class="flex flex-wrap gap-3">
          <a v-if="project.githubUrl" :href="project.githubUrl" target="_blank" rel="noopener" class="btn-secondary">GitHub</a>
          <a v-if="project.liveUrl" :href="project.liveUrl" target="_blank" rel="noopener" class="btn-primary">Live Demo</a>
    ...
    <section v-if="hasProblem || hasSolution || hasArchitecture" class="grid gap-6 lg:grid-cols-3">
        <div v-if="hasProblem" class="card space-y-3">
        ...
        <div v-if="hasSolution" class="card space-y-3">
        ...
        <div v-if="hasArchitecture" class="card space-y-3">
    ...
    <section v-if="hasResponsibilities" class="space-y-6">
      ...
      <ul class="grid gap-3 text-slate-600 dark:text-slate-300">
        <li
          v-for="item in project.details.responsibilities.filter((entry) => entry && entry.trim().length > 0)"
          :key="item"
          class="card"
        >
          {{ item }}
